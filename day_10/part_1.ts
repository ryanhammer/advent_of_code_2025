...
function parseMachineInput (machineInput: any[]): ParsedMachine {
  const inputCopy = [...machineInput];

  const lightDiagram = inputCopy.shift();
  const joltages = inputCopy.pop();
  const rawButtons = [...inputCopy];

  const buttons = rawButtons.map((rawButton: number[]) => {
    return Array.from({length: lightDiagram[0].length}, (_, i) => rawButton.includes(i));
  });
...
function arraysAreEqual(a: boolean[], b: boolean[]) {
  return a.length === b.length && a.every((val, i) => val === b[i]);
}

// Helper: Generate all combinations of size k from array of indices
...
function findMinimalSwitches(buttons: boolean[][], target: boolean[]) {
  const numButtons = buttons.length;
  const len = target.length;

  if (!buttons.every(arr => arr.length === len)) {
    throw new Error("All arrays must have the same length as target");
  }

  for (let k = 1; k <= numButtons; k++) {
    const combos = generateCombinations(Array.from({length: numButtons}, (_, i) => i), k);

    for (const combo of combos) {
      const combined = target.map((_, i) => {
        let count = 0;
        for (const idx of combo) {
          if (buttons[idx][i]) count++;
        }
        return count % 2 === 1;
      });

      if (arraysAreEqual(combined, target)) {
        return k;
      }
    }
  }
...
function determineLeastButtonPresses (machines: any[]) {
  let buttonPresses = 0;
  const parsedMachines = machines.map(machine => parseMachineInput(machine));

  parsedMachines.forEach((parsedMachine) => {
    const booleanLightDiagram = Array.from(parsedMachine.lightDiagram[0], (char => char === "#"));

    buttonPresses += findMinimalSwitches(parsedMachine.buttons, booleanLightDiagram);
  })

  return buttonPresses;
}

console.log(determineLeastButtonPresses(exampleMachines));
console.log(determineLeastButtonPresses(puzzleMachines));
