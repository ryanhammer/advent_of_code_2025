...
function determineInvalidIds (idRanges: string[]): number {
    let invalidIdSum = 0;
    ...
    idRanges.forEach((range) => {
        ...
        const startValue = Number(rangeParts[0].slice(0, Math.floor(rangeParts[0].length / 2)));
        const endValue = Number(rangeParts[1].slice(0, Math.floor(rangeParts[1].length / 2)));

        for (let i = startValue; i<=endValue; i++) {
            const valueToTest = Number(i.toString() + i.toString());
            
            if (valueToTest >= lowerLimit && valueToTest <= upperLimit) {
                invalidIdSum += valueToTest;
            }
        }
    })
...
console.log(determineInvalidIds(idRanges));
