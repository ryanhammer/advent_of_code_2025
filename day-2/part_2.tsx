...
function determineIdInvalid (id: number): boolean {
  const idString = id.toString();
  const idStringLength = idString.length;

  for (let i = 1; i <= Math.floor(idStringLength / 2); i++) {
    if (idStringLength % i !== 0) continue;

    const idSubstringToTest = idString.slice(0, i);
    let isInvalid = true;

    for (let j = i; j < idStringLength; j+=i) {
      if (idSubstringToTest !== idString.slice(j, j + i)) {
        isInvalid = false;
        break;
      }
    }
